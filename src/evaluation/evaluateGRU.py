...
Benchmark the trained model on WCEB (Bevendorff et al., 2023), reporting three things:
  ...
  3. Extraction throughput: pages/sec + mean/median sec per page, with hardware/device.
    ...
    How to run: python -m src.evaluation.evaluateGRU --model model.pt \
...
from src.models.gru.trainGRU import build_model
...
def main():
    ...
    ap.add_argument("--config", default=None,
                    help="run dir's config.json (hidden_dim/dropout/num_layers) -- required "
                         ...
                         "default-hyperparameter BiGRUTagger() and load_state_dict will raise "
    ...
    if args.config:
        ...
        model = BiGRUTagger().to(device)
    model.load_state_dict(torch.load(args.model, map_location=device))
    model.eval()
    n_params = sum(p.numel() for p in model.parameters())
    print(f"device={device}  BiGRU params={n_params:,}")
    ...
    summary = run_eval(model, args.wceb, datasets=args.datasets, n=args.n, threshold=args.threshold,
                        ...
                        arch="gru", out_basename=args.out)
